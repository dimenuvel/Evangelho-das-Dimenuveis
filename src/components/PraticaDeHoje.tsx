...
import { getTranslatedPractice, getTranslatedGiro } from '../utils/dataI18n';
import { getGiroById } from '../data/girosData';

export const PraticaDeHoje: React.FC = () => {
  const { getPraticaDeHoje, openPracticeTimer, navigateTo, completedPractices, t, language } = useApp();
  ...
  if (!hoje) {
    return (
      <div className="bg-[#0b0f19] border border-[#c5a059]/30 rounded-lg p-6 text-center">
        ...
        <h3 className="font-serif text-lg text-[#f3e3a2] font-bold">
          {language === 'en' ? 'Spiral Practice Complete' : 'Prática da Espiral Completa'}
        </h3>
        <p className="text-xs text-neutral-400 mt-1 max-w-md mx-auto">
          {language === 'en'
            ? 'You have completed all active practices. Continue your contemplative journey or revisit previous Turns.'
  ...
  const { practice: rawPractice, giroNumber } = hoje;
  const practice = getTranslatedPractice(rawPractice, language);
  const rawGiro = getGiroById(rawPractice.giroId);
  const translatedGiro = rawGiro ? getTranslatedGiro(rawGiro, language) : null;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
        <div className="space-y-3 flex-1">
          ...
          <div className="flex items-center gap-4 text-xs text-neutral-400 pt-1">
            <div className="flex items-center gap-1.5 bg-[#07090e]/60 px-3 py-1 rounded-md border border-neutral-800">
              ...
              <span>{practice.suggestedDurationMinutes} {t('praticaHoje.min')} {language === 'en' ? 'suggested' : 'sugeridos'}</span>
            </div>

            {isAlreadyDone && (
              <div className="flex items-center gap-1 text-emerald-400 font-medium">
                <CheckCircle2 className="w-4 h-4" />
                <span>{t('praticaHoje.completedTag')}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          >
            ...
            <span>{t('praticaHoje.startBtn')} ({practice.suggestedDurationMinutes} {t('praticaHoje.min')})</span>
          </button>
          ...
          >
            ...
            <span>{language === 'en' ? 'View Turns' : 'Ver Giros da Espiral'}</span>
          </button>
        </div>
      </div>
    </div>
